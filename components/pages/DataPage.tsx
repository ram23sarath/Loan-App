...
const DataPage = () => {
  ...
  const [sortColumn, setSortColumn] = useState<
    "date" | "type" | "subtype" | "amount" | "receipt" | "notes" | null
  >("date");
  ...
  const customerEntryGroups = useMemo(() => {
    const map = new Map<
      string,
      { customerId: string; name: string; entries: typeof dataEntries }
    >();
    displayedDataEntries.forEach((entry) => {
      ...
      if (!map.has(key)) {
        map.set(key, {
          customerId: key,
          name: customerMap.get(key) || "Unknown",
          entries: [],
        });
      }
  ...
  const sortedEntries = useMemo(() => {
    ...
    const sorted = [...selectedGroupEntries].sort((a, b) => {
      ...
      if (sortColumn === "date") {
        const timeA = new Date(compareA).getTime();
        const timeB = new Date(compareB).getTime();
  ...
  // --- EFFECTS ---
  // Reset selected customer when component mounts (page navigation)
  useEffect(() => {
    // For scoped customers, keep their selection. For admin users, clear selection.
    if (!isScopedCustomer) {
      setSelectedCustomerId(null);
    }
  }, [isScopedCustomer]);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      try {
        document.querySelector<HTMLInputElement>("#edit-amount")?.focus();
      } catch (e) {
        /* ignore */
      }
    }, 80);
  ...
  useEffect(() => {
    const handleClickOutsideNotes = (event: MouseEvent) => {
      if (
        expandedNoteId &&
        notesRefs.current[expandedNoteId] &&
        !notesRefs.current[expandedNoteId]?.contains(event.target as Node)
      ) {
    ...
    return () =>
      document.removeEventListener("mousedown", handleClickOutsideNotes);
  }, [expandedNoteId]);
  ...
  if (isScopedCustomer && displayedDataEntries.length === 0) {
    const customerName = scopedCustomerId
                    ...
                    <div className="hidden md:block md:w-1/3 lg:w-1/4 bg-white border border-gray-200 rounded-lg shadow-sm p-3 dark:bg-dark-card dark:border-dark-border">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-xs text-gray-500 dark:text-dark-muted">
                          {filteredCustomerGroups.length}
                        </span>
                      </div>
                      {customerEntryGroups.length === 0 ? (
                        <div className="text-center text-gray-500 py-10 text-sm dark:text-dark-muted">
                                ...
                                return (
                                  ...
                                  >
                                    <div className="flex items-center justify-between gap-2">
                                      ...
                                      <div className="text-xs text-gray-500 dark:text-dark-muted whitespace-nowrap">
                        ...
                        >
                          <thead className="bg-indigo-50 dark:bg-indigo-900/30">
                            <tr className="text-left text-xs font-bold text-indigo-700 uppercase tracking-wider dark:text-indigo-400">
                              ...
                              <th
                                className="px-4 py-3 text-left cursor-pointer hover:bg-indigo-100 dark:hover:bg-indigo-900/50"
                              ...
                              <th
                                className="px-4 py-3 text-center cursor-pointer hover:bg-indigo-100 dark:hover:bg-indigo-900/50"
                                style={{ minWidth: "60px" }}
                              ...
                              <th
                                className="px-4 py-3 cursor-pointer hover:bg-indigo-100 dark:hover:bg-indigo-900/50"
                                style={{ minWidth: "80px" }}
                                onClick={() => handleSortColumn("subtype")}
                              >
                              ...
                              <th
                                className="px-4 py-3 cursor-pointer hover:bg-indigo-100 dark:hover:bg-indigo-900/50"
                                style={{ minWidth: "90px" }}
                                onClick={() => handleSortColumn("receipt")}
                              >
                                Receipt #{getSortIndicator("receipt")}
                              </th>
                              <th
                                className="px-4 py-3 cursor-pointer hover:bg-indigo-100 dark:hover:bg-indigo-900/50"
                                style={{ minWidth: "180px" }}
                                onClick={() => handleSortColumn("notes")}
                              >
                                Notes{getSortIndicator("notes")}
                              </th>
                              {!isScopedCustomer && (
                                <th
                                  className="px-4 py-3 text-center text-red-600 dark:text-red-400"
                          ...
                          <tbody className="bg-white dark:bg-dark-card">
                            {!selectedCustomerId ||
                              ...
                              <tr>
                                <td
                                  colSpan={8}
                                  className="text-center text-gray-500 py-16 text-base dark:text-dark-muted"
                                >
                                  {!selectedCustomerId
                              ...
                              paginatedEntries.map((entry, idx) => {
                                ...
                                return (
                                  ...
                                  >
                                    <td className="px-4 py-3 text-gray-500 text-left dark:text-dark-muted">
                                      {actualIndex}
                                    </td>
                                    <td className="px-4 py-3 text-gray-600 text-left dark:text-dark-muted">
                                    ...
                                    <td className="px-4 py-3 text-center">
                                      {entry.type === "credit" ? (
                                        ...
                                        <span className="inline-block px-2 py-1 rounded-full bg-red-100 text-red-800 font-semibold text-xs dark:bg-red-900/30 dark:text-red-400">
                                          Expense
                                        </span>
                                      )}
                                    </td>
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="flex items-start justify-between gap-3 mb-3">
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <h3 className="font-bold text-sm sm:text-base text-gray-900 dark:text-dark-text truncate">
                                        {customerMap.get(entry.customer_id) ||
                                          "Unknown"}
                                      </h3>
                                    </div>
                                  ...
                                  <div className="grid grid-cols-2 gap-3 py-3 border-t border-b border-gray-100 dark:border-slate-700/50 mb-3">
                                    <div>
                                      <span className="text-xs font-semibold text-gray-500 dark:text-dark-muted block mb-1">
                                        Date
                                      </span>
                ...
                >
                  {isScopedCustomer ? (
                    <div className="flex flex-col items-center justify-center h-full py-12 text-center">
                      <p className="text-gray-600 text-lg mb-4 dark:text-dark-muted">
                        You don't have permission to add data entries.
                      </p>
                      <p className="text-gray-500 dark:text-dark-muted">
                        As a customer, you can only view your own entries.
                      </p>
                    </div>
                  ) : (
                    <form
                      className="flex flex-col gap-4 w-full p-1 pb-2 flex-1"
                      onSubmit={handleSubmit}
                    >
                      <div className="relative" ref={customerDropdownRef}>
                        <label
                          htmlFor="customer-btn"
                        ...
                        <button
                          id="customer-btn"
                          type="button"
                          className={`${inputBaseStyle} flex justify-between items-center text-left bg-white dark:bg-slate-700`}
                        ...
                        <AnimatePresence>
                          {showCustomerDropdown && (
                            <motion.div
                              ...
                              animate="visible"
                              exit="exit"
                              className="absolute top-full left-0 z-20 w-full bg-white border border-gray-300 rounded-lg shadow-lg max-h-52 overflow-y-auto mt-1 scrollbar-thin dark:bg-dark-card dark:border-dark-border"
                            >
                              <div className="p-2 sticky top-0 bg-white z-10 border-b border-gray-200 dark:bg-dark-card dark:border-dark-border">
                                <input
                                  ...
                                  className="p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-200 text-sm w-full dark:bg-slate-700 dark:border-dark-border dark:text-dark-text dark:placeholder-dark-muted"
                                  autoFocus
                                />
                              </div>
                              {filteredCustomers.length === 0 ? (
                                <div className="p-2 text-gray-400 text-sm dark:text-dark-muted">
                                  No customers found.
                                </div>
                              ) : (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                        ...
                        <div className="md:col-span-2">
                          <label htmlFor="subtype" className={labelBaseStyle}>
                            Subtype
                          </label>
                          <select
                            id="subtype"
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          value={form.notes}
                          onChange={handleChange}
                          className={`${inputBaseStyle} min-h-[80px] resize-y`}
                          rows={2}
                    ...
                    <div className="flex-1 overflow-y-auto scrollbar-thin space-y-2">
                      ...
                      >
                        ...
                        <div className="text-xs text-gray-500 dark:text-dark-muted">
                          Show entries from all customers
                        </div>
                      </button>
              ...
              {editEntryId && (
                ...
                >
                  <motion.div
                    ...
                    animate="visible"
                    exit="exit"
                    className="bg-white rounded-xl shadow-lg p-4 w-full max-w-sm max-h-[90vh] overflow-y-auto mx-4 scrollbar-thin dark:bg-dark-card dark:border dark:border-dark-border"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <div
                      id="edit-entry-modal-title"
                      className="text-lg font-semibold text-gray-800 mb-4 dark:text-dark-text"
                    >
                      Edit Entry
                    </div>
                    <div className="grid grid-cols-1 gap-3">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          // Validate year boundaries (YYYY-MM-DD format) - only for complete dates
                          if (newDate) {
                            // Only validate if the date is in complete YYYY-MM-DD format
              ...
              {deleteId && (
                ...
                >
                  ...
                  >
                    ...
                    <p className="text-gray-600 mb-6 text-center dark:text-dark-muted">
