            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="border-b border-gray-200 pb-4">
                  ...
                  <p className="text-sm font-semibold text-gray-800 mt-1">{userEmail}</p>
                </div>
                ...
                {isScopedCustomer && customerDetails ? (
                  <>
                    <div className="border-b border-gray-200 pb-4">
                      ...
                      <p className="text-sm font-semibold text-gray-800 mt-1">{customerDetails.name || '—'}</p>
                    </div>

                    <div className="border-b border-gray-200 pb-4">
                      <label className="text-xs text-gray-500 uppercase tracking-wide">Phone Number</label>
                      <p className="text-sm font-semibold text-gray-800 mt-1">{customerDetails.phone || '—'}</p>
                    </div>

                    {customerDetails.address && (
                      <div className="border-b border-gray-200 pb-4">
                        <label className="text-xs text-gray-500 uppercase tracking-wide">Address</label>
                        <p className="text-sm font-semibold text-gray-800 mt-1">{customerDetails.address}</p>
                      </div>
                    )}
                      ...
                      {isEditingStation ? (
                        <div className="space-y-2">
                          <input
                            ...
                            className="w-full bg-gray-50 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                          />
                          <button
                            onClick={handleSaveStation}
                            disabled={isSavingStation}
                            className="w-full px-3 py-1 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white text-sm font-medium rounded transition-colors"
                          >
                            {isSavingStation ? 'Saving...' : 'Save'}
                          </button>
                        </div>
                      ) : (
                        <p className="text-sm font-semibold text-gray-800 mt-1">{stationName || '—'}</p>
                      )}
                    </div>
                  </>
                ) : isScopedCustomer ? (
                  ...
                  <div className="text-center py-4">
                    <p className="text-sm text-gray-500">Admin accounts do not have customer details</p>
                  </div>
                )}
              </div>
