...
import AvatarReminderModal from "./components/modals/AvatarReminderModal";
...
const AppShell = ({
  ...
  const reminderSessionKey = React.useMemo(() => {
    if (!currentUserId) return null;
    return `loan_app_avatar_reminder_shown_${currentUserId}`;
  }, [currentUserId]);

  React.useEffect(() => {
    if (!currentUserId || hasAvatar || !reminderSessionKey) {
      setShowAvatarReminder(false);
      return;
    }

    let hasSeenReminderThisSession = false;
    try {
      hasSeenReminderThisSession =
        window.sessionStorage.getItem(reminderSessionKey) === "1";
    } catch {
      hasSeenReminderThisSession = false;
    }

    if (hasSeenReminderThisSession) {
      setShowAvatarReminder(false);
      return;
    }

    try {
      window.sessionStorage.setItem(reminderSessionKey, "1");
    } catch {
      // Ignore storage failures and continue showing reminder in-memory.
    }
    setShowAvatarReminder(true);
  }, [currentUserId, hasAvatar, reminderSessionKey]);
