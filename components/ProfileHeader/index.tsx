...
const ProfileHeader = forwardRef<ProfileHeaderHandle>((props, ref) => {
    ...
    const { session, signOut, isScopedCustomer, customerMap, scopedCustomerId, updateCustomer } = useData();
    ...
    const notifications = useNotifications(isScopedCustomer);
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            if (e.key === 'Escape') {
                ...
                } else if (backup.backupRunning && (backup.backupProgress >= 100 || backup.backupCurrentStep.startsWith('❌'))) {
                    backup.closeBackup();
                }
            }
        };
    ...
    }, [showMenu, showChangePasswordModal, showToolsModal, showProfilePanel, showLogoutConfirm, showNotificationModal, backup]);
