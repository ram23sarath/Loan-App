...
const SubscriptionTableView: React.FC<SubscriptionTableViewProps> = ({
  ...
  const [pagePickerOpen, setPagePickerOpen] = React.useState<
    "start" | "end" | null
  >(null);
  const [startPagePickerOffset, setStartPagePickerOffset] = React.useState(0);
  const [endPagePickerOffset, setEndPagePickerOffset] = React.useState(0);
      ...
      <table className="min-w-full border-collapse hidden md:table">
        <thead>
          <tr className="bg-gray-100 dark:bg-slate-700">
            <th className="px-4 py-2 border-b text-left dark:border-dark-border dark:text-dark-text">
              Sr.No
            </th>
            <th
            ...
            {!isScopedCustomer && (
              <th className="px-4 py-2 border-b text-center dark:border-dark-border dark:text-dark-text whitespace-nowrap">
                Actions
              </th>
            )}
          </tr>
            ...
            {paginatedSubscriptions.map((sub, idx) => {
              ...
              if (
                ...
                message = `Hi ${customer.name}, your subscription payment of ${formatCurrencyIN(
                  sub.amount,
                )} was received on ${formatDate(sub.date, "whatsapp")}${
              ...
              >
                ...
                >
                  ...
                  <div className="text-xs text-gray-500 mt-1 dark:text-dark-muted">
                    Date: {sub.date ? formatDate(sub.date) : "-"}
                  </div>
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-4 pt-4 border-t border-gray-200 dark:border-dark-border">
          <div className="text-sm text-gray-600 dark:text-dark-muted">
            ...
            {Math.min(currentPage * itemsPerPage, sortedSubscriptions.length)}{" "}
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => {
              ...
              if (page === 2 && currentPage > 3) {
                ...
                const visiblePages = startPages.slice(
                  startPagePickerOffset * 9,
                  (startPagePickerOffset + 1) * 9,
                );
                  ...
                  <div key="dots-start" className="relative">
                    <button
                      onClick={() => {
                        ...
                        setPagePickerOpen(nextOpen);
                        if (nextOpen === "start") setStartPagePickerOffset(0);
                      }}
                    ...
                    {pagePickerOpen === "start" && (
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-lg shadow-lg p-2 z-50 min-w-[140px]">
                        ...
                        <div className="flex items-center justify-between mb-2 px-1">
                          <button
                            onClick={() =>
                              setStartPagePickerOffset((currentOffset) =>
                                Math.max(0, currentOffset - 1),
                              )
                            }
                            disabled={startPagePickerOffset === 0}
                          ...
                          <button
                            ...
                            disabled={startPagePickerOffset >= maxOffset}
                            className="p-1 text-gray-500 dark:text-dark-muted hover:text-indigo-600 dark:hover:text-indigo-400 disabled:opacity-30 disabled:cursor-not-allowed"
                          >
                            ›
                          </button>
              ...
              if (page === totalPages - 1 && currentPage < totalPages - 2) {
                ...
                const visiblePages = endPages.slice(
                  endPagePickerOffset * 9,
                  (endPagePickerOffset + 1) * 9,
                );
                  ...
                  <div key="dots-end" className="relative">
                    <button
                      onClick={() => {
                        ...
                        setPagePickerOpen(nextOpen);
                        if (nextOpen === "end") setEndPagePickerOffset(0);
                      }}
                      className="px-2 text-gray-500 dark:text-dark-muted hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-gray-100 dark:hover:bg-slate-700 rounded"
                      title="Click to show more pages"
                    >
                    ...
                    {pagePickerOpen === "end" && (
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-lg shadow-lg p-2 z-50 min-w-[140px]">
                        ...
                        <div className="flex items-center justify-between mb-2 px-1">
                          <button
                            onClick={() =>
                              setEndPagePickerOffset((currentOffset) =>
                                Math.max(0, currentOffset - 1),
                              )
                            }
                            disabled={endPagePickerOffset === 0}
                            className="p-1 text-gray-500 dark:text-dark-muted hover:text-indigo-600 dark:hover:text-indigo-400 disabled:opacity-30 disabled:cursor-not-allowed"
                          >
                            ‹
                          </button>
                          <span className="text-xs text-gray-500 dark:text-dark-muted">
                            {endPagePickerOffset * 9 + 1}-
                            {Math.min(
                              (endPagePickerOffset + 1) * 9,
                              endPages.length,
                            )}{" "}
                          ...
                          <button
                            ...
                            disabled={endPagePickerOffset >= maxOffset}
                            className="p-1 text-gray-500 dark:text-dark-muted hover:text-indigo-600 dark:hover:text-indigo-400 disabled:opacity-30 disabled:cursor-not-allowed"
                          >
                            ›
                          </button>
