...
const CustomerDetailPage: React.FC = () => {
  ...
  if (loading) {
    return (
      <PageWrapper>
        <GlassCard><p className="dark:text-dark-text">Loading...</p></GlassCard>
      </PageWrapper>
    );
  }
  ...
  const customerSubscriptions = subscriptions.filter(s => s.customer_id === id);
  ...
  return (
    <PageWrapper>
      <div className="mb-4">
        <button
          onClick={() => navigate('/customers')}
          className="px-4 py-2 rounded bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-dark-text hover:bg-gray-300 dark:hover:bg-slate-600"
        >
          ← Back to Customers
        </button>
      </div>
