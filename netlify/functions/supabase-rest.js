...
export default async (req) => {
  ...
  try {
    ...
    const targetUrl = `${SUPABASE_URL}/rest/v1/${pathSuffix}${forwardQuery}`;
    ...
    if (debug) {
      ...
      console.log('[supabase-rest-proxy] forwarding', { method: req.method, path: requestUrl.pathname, targetUrl, headers: safeReqHeaders });
    }
    ...
    if (debug) {
      ...
      console.log(`[supabase-rest-proxy] response status=${response.status} statusText=${response.statusText} content-type=${respType} elapsed=${elapsed}ms`);
      if (verbose) {
        try {
          const cloned = response.clone();
          const text = await cloned.text();
          const snippet = text.length > 1024 ? text.slice(0, 1024) + '...<truncated>' : text;
          console.log('[supabase-rest-proxy] response body snippet:\n' + snippet);
        } catch (e) {
          console.log('[supabase-rest-proxy] failed to read response body for debug:', e?.message || e);
        }
      }
    }
